...
4. **Credenciais de Rede e Provisionamento:**
   O Wi-Fi pode ser configurado via credenciais *hardcoded* (para testes na planta) ou dinamicamente. O sistema utiliza **mDNS** para se anunciar na rede, permitindo que a Web IDE descubra o nó automaticamente e realize o provisionamento (renomeação) via *Hot-Deploy*.
...
* **E-Cycle (`e_cycle_block`)**: Atua como o gerador de *ticks* ou *clock* cíclico. Ele dispara eventos ("EVENT_OUT") em intervalos regulares configurados, servindo de gatilho principal para os fluxos IEC 61499.
